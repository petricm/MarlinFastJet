 ...
 * make FastJetUtil a friend class of your Processor (registerProcessorParameter
 is protected so we need to be a friend)
 ...
 *   inline PseudoJetList clusterJets(PseudoJetList& pjList, LCCollection*
 reconstructedPars);
 *
 */
...
void FastJetUtil::registerFastJetParameters( T* proc ) {
  ...
  proc->registerProcessorParameter( "algorithm",
                                    "Selects the algorithm and its parameters. E.g. 'kt_algorithm 0.7' or "
                                    "'ee_kt_algorithm'. For a full list of supported algorithms, see the "
                                    "logfile after execution.",
                                    _jetAlgoNameAndParams, defAlgoAndParam );

  proc->registerProcessorParameter( "recombinationScheme",
                                    "The recombination scheme used when merging 2 particles. Usually there "
                                    "is no need to use anything else than 4-Vector addition: E_scheme",
                                    _jetRecoSchemeName, std::string( "E_scheme" ) );

  EVENT::StringVec defClusterMode;
  defClusterMode.push_back( "Inclusive" );
  proc->registerProcessorParameter( "clusteringMode",
...
void FastJetUtil::initJetAlgo() {
  ...
  if ( !_jetAlgo ) {
    streamlog_out( ERROR ) << "The given algorithm \"" << _jetAlgoName << "\" is unknown to me!" << std::endl;
...
void FastJetUtil::initRecoScheme() {
  if ( _jetRecoSchemeName.compare( "E_scheme" ) == 0 )
    ...
    streamlog_out( ERROR ) << "Unknown recombination scheme: " << _jetRecoSchemeName << std::endl;
...
PseudoJetList FastJetUtil::clusterJets( PseudoJetList& pjList, LCCollection* reconstructedPars ) {
  ...
  if ( _clusterMode == FJ_inclusive ) {
    ...
    if ( reconstructedPars->getNumberOfElements() < (int)_requestedNumberOfJets ) {
      streamlog_out( WARNING ) << "Not enough elements in the input collection to create " << _requestedNumberOfJets
                               << " jets." << std::endl;
...
PseudoJetList FastJetUtil::convertFromRecParticle( LCCollection* recCol ) {
  ...
  for ( int i = 0; i < recCol->getNumberOfElements(); ++i ) {
    ...
    pjList.push_back( fastjet::PseudoJet( par->getMomentum()[ 0 ], par->getMomentum()[ 1 ], par->getMomentum()[ 2 ],
                                          par->getEnergy() ) );
