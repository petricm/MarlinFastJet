...
FastJetProcessor::FastJetProcessor() : Processor( "FastJetProcessor" ) {
  ...
  registerProcessorParameter( "algorithm",
                              "Selects the algorithm and its parameters. E.g. "
                              "'kt_algorithm 0.7' or 'ee_kt_algorithm'. For a "
                              "full list of supported algorithms, see the "
                              "logfile after execution.",
                              _jetAlgoNameAndParams, defAlgoAndParam );

  registerProcessorParameter( "recombinationScheme",
                              "The recombination scheme used when merging 2 "
  ...
  registerProcessorParameter( "clusteringMode",
                              "One of 'Inclusive <minPt>', 'InclusiveIterativeNJets <nrJets> <minE>', "
                              "'ExclusiveNJets <nrJets>', 'ExclusiveYCut <yCut>'. Note: not all modes "
                              "are available for all algorithms.",
                              _clusterModeNameAndParam, defClusterMode );
}
...
void FastJetProcessor::initClusterMode() {
  ...
  // check the different cluster mode possibilities, and check if the number of
  ...
  if ( _clusterModeName.compare( "Inclusive" ) == 0 ) {
    if ( _clusterModeNameAndParam.size() != 2 )
      throw Exception(
          ...
          "name=\"clusteringMode\" type=\"StringVec\"> Inclusive <minPt> "
          "</parameter>" );
    ...
    if ( _clusterModeNameAndParam.size() != 3 )
      throw Exception(
          "Wrong Parameter(s) for Clustering Mode. Expected:\n <parameter "
          "name=\"clusteringMode\" type=\"StringVec\"> InclusiveIterativeNJets "
          "<NJets> <minE> </parameter>" );
    ...
    if ( _clusterModeNameAndParam.size() != 2 )
      throw Exception(
          "Wrong Parameter(s) for Clustering Mode. Expected:\n <parameter "
          "name=\"clusteringMode\" type=\"StringVec\"> ExclusiveNJets <NJets> "
          "</parameter>" );
    _nrJets      = atoi( _clusterModeNameAndParam[ 1 ].c_str() );
    ...
    if ( _clusterModeNameAndParam.size() != 2 )
      throw Exception(
          "Wrong Parameter(s) for Clustering Mode. Expected:\n <parameter "
          "name=\"clusteringMode\" type=\"StringVec\"> ExclusiveYCut <yCut> "
          "</parameter>" );
...
void FastJetProcessor::initJetAlgo() {
  ...
  _jetAlgo = NULL;
  streamlog_out( MESSAGE ) << "Algorithms: ";  // the isJetAlgo function will
                                               // write to
                                               // streamlog_out(MESSAGE), so
                                               // that we get a list of
                                               // available algorithms in the
                                               // log

  // example: kt_algorithm, needs 1 parameter, supports inclusive,
  // inclusiveIterative, exlusiveNJets and exlusiveYCut clustering
  if ( isJetAlgo( "kt_algorithm", 1,
                  FJ_inclusive | FJ_exclusive_nJets | FJ_exclusive_yCut | OWN_inclusiveIteration ) ) {
    _jetAlgoType = kt_algorithm;
    _jetAlgo     = new fastjet::JetDefinition( _jetAlgoType, atof( _jetAlgoNameAndParams[ 1 ].c_str() ), _jetRecoScheme,
                                           _strategy );
  }

  if ( isJetAlgo( "cambridge_algorithm", 1,
                  FJ_inclusive | FJ_exclusive_nJets | FJ_exclusive_yCut | OWN_inclusiveIteration ) ) {
  ...
  if ( isJetAlgo( "genkt_algorithm", 2,
                  FJ_inclusive | OWN_inclusiveIteration | FJ_exclusive_nJets | FJ_exclusive_yCut ) ) {
    _jetAlgoType = genkt_algorithm;
    _jetAlgo     = new fastjet::JetDefinition( _jetAlgoType, atof( _jetAlgoNameAndParams[ 1 ].c_str() ),
                                           atof( _jetAlgoNameAndParams[ 2 ].c_str() ), _jetRecoScheme, _strategy );
  }

  if ( isJetAlgo( "cambridge_for_passive_algorithm", 1,
                  FJ_inclusive | OWN_inclusiveIteration | FJ_exclusive_nJets | FJ_exclusive_yCut ) ) {
  ...
  if ( isJetAlgo( "ee_kt_algorithm", 0, FJ_exclusive_nJets | FJ_exclusive_yCut ) ) {
    ...
    _jetAlgo     = new fastjet::JetDefinition( _jetAlgoType, _jetRecoScheme, _strategy );
  }
  ...
  if ( isJetAlgo( "SISConePlugin", 2, FJ_inclusive | OWN_inclusiveIteration ) ) {
    ...
    pl = new fastjet::SISConePlugin( atof( _jetAlgoNameAndParams[ 1 ].c_str() ),
  ...
  if ( isJetAlgo( "SISConeSphericalPlugin", 2, FJ_inclusive | OWN_inclusiveIteration ) ) {
    ...
    pl = new fastjet::SISConeSphericalPlugin( atof( _jetAlgoNameAndParams[ 1 ].c_str() ),
                                              atof( _jetAlgoNameAndParams[ 2 ].c_str() ) );
  ...
  if ( isJetAlgo( "ValenciaPlugin", 3, FJ_exclusive_nJets | FJ_exclusive_yCut ) ) {
    ...
    pl = new fastjet::contrib::ValenciaPlugin( atof( _jetAlgoNameAndParams[ 1 ].c_str() ),  // R value
  ...
  if ( !_jetAlgo ) {
    streamlog_out( ERROR ) << "The given algorithm \"" << _jetAlgoName << "\" is unknown to me!" << endl;
...
bool FastJetProcessor::isJetAlgo( string algo, int nrParams, int supportedModes ) {
  ...
  streamlog_out( MESSAGE ) << "*";  // mark the current algorithm as the
                                    // selected one, even before we did our
                                    // checks on nr of parameters etc.
  ...
  if ( (int)_jetAlgoNameAndParams.size() - 1 != nrParams ) {
    streamlog_out( ERROR ) << endl
                           ...
                           << "We need " << nrParams << " params, but we got " << _jetAlgoNameAndParams.size() - 1
                           << endl;
    throw Exception(
        "You have insufficient number of parameters for this algorithm! See "
  ...
  if ( ( supportedModes & _clusterMode ) != _clusterMode ) {
    streamlog_out( ERROR ) << endl
                           << "This algorithm is not capable of running in this clustering mode (" << _clusterMode
                           << "). Sorry!" << endl;
...
void FastJetProcessor::initRecoScheme() {
  ...
  if ( _jetRecoSchemeName.compare( "E_scheme" ) == 0 )
    ...
    streamlog_out( ERROR ) << "Unknown recombination scheme: " << _jetRecoSchemeName << endl;
...
void FastJetProcessor::initStrategy() {
  ...
  // however, when using the 'Best' clustering strategy FJ will chose
  // automatically from a big list
  // changing this would (most likely) only change the speed of calculation, not
  // the outcome
...
void FastJetProcessor::processEvent( LCEvent* evt ) {
  // clear the list containing the (pseudo)-jets and the
  // FJ-index-to-PFOReconstructedParticle map
  _pjList.clear();
  ...
  if ( _clusterMode == FJ_inclusive ) {
    ...
    if ( _reconstructedPars->getNumberOfElements() < (int)_nrJets ) {
      streamlog_out( WARNING ) << "Not enough elements in the input collection to create " << _nrJets << " jets."
                               << endl;
...
vector<fastjet::PseudoJet> FastJetProcessor::doIterativeInclusiveClustering() {
  ...
  // for that we will do inclusive clustering, modifying the R parameter in some
  // kind of minimization
  ...
  double RDiff = R / 2;   // the step size we modify the R parameter at each
                          // iteration. Its size for the n-th step is R/(2n),
                          // i.e. starts with R/2
  ...
  // This is necessary, as these are plugins and hence use a different
  // constructor than
  // the built in fastjet algorithms

  // here we save pointer to the plugins, so that if they are created, we can
  // delete them again after usage
  ...
  // For each iteration we will modify the Radius R by RDiff. RDiff will be
  // reduced by 2 for each iteration
  ...
  for ( iIter = 0; iIter < ITERATIVE_INCLUSIVE_MAX_ITERATIONS; iIter++ ) {
    // do the clustering for this value of R. For this we need to re-initialize
    // the JetDefinition, as it takes the R parameter
    JetDefinition* def = NULL;

    // unfortunately SisCone(spherical) are being initialized differently, so we
    ...
    if ( nJets == _nrJets ) {  // if the number of jets is correct: success!
      break;
    } else if ( nJets < _nrJets ) {  // if number of jets is too small: we need
                                     // a smaller Radius per jet (so that we get
                                     // more jets)
      R -= RDiff;
    } else if ( nJets > _nrJets ) {  // if the number of jets is too high:
                                     // increase the Radius
      R += RDiff;
    }
  ...
  if ( iIter == ITERATIVE_INCLUSIVE_MAX_ITERATIONS ) {
    ...
    // Currently we will return the latest results, independent if the number is
    // actually matched
    // jetsReturn.clear();
  }
...
void FastJetProcessor::convertFromRecParticle( LCCollection* recCol ) {
  // foreach RecoParticle in the LCCollection: convert it into a PseudoJet and
  ...
  for ( int i = 0; i < recCol->getNumberOfElements(); ++i ) {
    ...
    _pjList.push_back( fastjet::PseudoJet( par->getMomentum()[ 0 ], par->getMomentum()[ 1 ], par->getMomentum()[ 2 ],
                                           par->getEnergy() ) );
